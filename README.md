...
- **FOUC 防止** --- ReslideEmbed で画像読み込み完了まで非表示、フェードインで滑らかに表示
- **画像エクスポート** --- PNG / JPG / WebP / AVIF 形式、1920x1080 キャプチャ → 任意サイズにリサイズ
- **PDF エクスポート** --- `Ctrl+P` で印刷 / PDF 保存、CLI からもエクスポート可能
- **serverExternalPackages 不要** --- ネイティブバイナリ依存なし（sucrase 使用）、Next.js で設定不要
...
コンパイル時（`compileMdxSlides()`）にトランスパイルしてインライン化されるため、ホストアプリ側での `components` prop 登録は不要。
...
| `--width <width>`     | 出力画像の幅（キャプチャは常に 1920） | `1920`     |
| `--height <height>`   | 出力画像の高（キャプチャは常に 1080） | `1080`     |
